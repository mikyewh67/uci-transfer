(() => {
  ...
  function defaultState(){
    return {
      ...
      settings:{targetGpa:3.70,primarySchool:"uci",mathPath:"math2-first"},
  ...
  function renderOptimizer(){
    ...
    byId("openMathCourseBtn")?.addEventListener("click",()=>{closeOverlay("optimizerOverlay");openCourse("math2")});
  }

  function buildCommandIndex(q=""){
    const items=[];state.courses.forEach(c=>items.push({title:`${c.code} — ${c.title}`,sub:`${TERMS.find(t=>t.id===c.term).label} · ${c.units} units · ${valueLabel(c)}`,kind:"COURSE",run:()=>openCourse(c.id),text:`${c.code} ${c.title} ${c.why} ${c.geTargets.join(" ")} ${c.required.join(" ")}`.toLowerCase()}));
    Object.entries(SCHOOLS).forEach(([id,s])=>items.push({title:`${s.name} — ${s.major}`,sub:`${requirementStatus(id).mapped}/${s.req.length} major-prep courses mapped`,kind:"CAMPUS",run:()=>{go("requirements");state.ui.reqMode="schools";save();renderRequirements();closeOverlay("commandOverlay")},text:`${s.name} ${s.major} ${s.note}`.toLowerCase()}));
    GE_AREAS.forEach(a=>items.push({title:`Cal-GETC ${a.id} — ${a.name}`,sub:geStatus(a).detail,kind:"GE",run:()=>{go("requirements");state.ui.reqMode="ge";save();renderRequirements();closeOverlay("commandOverlay")},text:`${a.id} ${a.name}`.toLowerCase()}));
    items.push({title:"Optimize saved plan",sub:"Check units, prerequisites, load and alternate math route",kind:"COMMAND",run:()=>{closeOverlay("commandOverlay");openOptimizer()},text:"optimize plan what if math"},{title:"Export plan JSON",sub:"Create a device-change backup",kind:"COMMAND",run:()=>exportJSON(),text:"export backup json"},{title:"Open settings",sub:"Primary campus, GPA target, decisions and data vault",kind:"COMMAND",run:()=>{closeOverlay("commandOverlay");openSettings()},text:"settings primary gpa"});
    const needle=q.trim().toLowerCase();return (needle?items.filter(i=>i.text.includes(needle)||i.title.toLowerCase().includes(needle)):items).slice(0,14);
  }
  function renderCommand(q=""){
    const items=buildCommandIndex(q);const out=byId("commandResults");out.innerHTML=items.length?items.map((i,n)=>`<button class="command-result" data-command="${n}"><span><strong>${esc(i.title)}</strong><span>${esc(i.sub)}</span></span><em>${i.kind}</em></button>`).join(""):`<div class="empty">No match. Try a course code, campus or Cal-GETC area.</div>`;out.querySelectorAll("[data-command]").forEach(b=>b.onclick=()=>items[+b.dataset.command].run());
  }
  ...
  function openOverlay(id){const x=byId(id);x.classList.add("open");x.setAttribute("aria-hidden","false")}
  function closeOverlay(id){const x=byId(id);x.classList.remove("open");x.setAttribute("aria-hidden","true")}
  ...
  function bindSettings(){
    byId("saveSettingsBtn").onclick=()=>{rememberUndo("settings");state.settings.primarySchool=byId("primarySchoolSelect").value;state.settings.targetGpa=Math.max(2,Math.min(4,parseFloat(byId("targetGpaInput").value)||3.7));state.settings.mathPath=byId("mathPathSelect").value;save();renderAll();closeOverlay("settingsOverlay");toast("Settings saved")};
    ...
    byId("resetBtn").onclick=()=>{if(!confirm("Reset the app to the verified v4 plan? Your saved grades, moves, notes and logs will be replaced."))return;localStorage.removeItem(STORAGE_KEY);state=defaultState();save();renderAll();closeOverlay("settingsOverlay");toast("Reset to v4 plan")};
  }
  ...
  function importJSON(file){const r=new FileReader();r.onload=()=>{try{const data=JSON.parse(r.result);if(!Array.isArray(data.courses))throw new Error();rememberUndo("import");state={...defaultState(),...data};save();renderAll();toast("Backup imported")}catch{toast("That file is not a valid 405 South backup")}};r.readAsText(file)}
  ...
  function counselorReport(){
    const uci=requirementStatus("uci"),ucla=requirementStatus("ucla"),ge=geSummary(),g=gpaData();const openMajor=SCHOOLS.uci.req.filter(id=>course(id)?.status!=="completed").map(id=>course(id)?.code).join(", ");
    const html=`<!doctype html><meta charset="utf-8"><title>405 South Counselor Report</title><style>body{font:14px system-ui;max-width:900px;margin:40px auto;padding:0 20px;color:#111}h1{margin-bottom:4px}h2{margin-top:26px;border-bottom:1px solid #ccc;padding-bottom:5px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:7px;text-align:left}small{color:#666}</style><h1>405 South — Counselor Report</h1><small>Generated ${new Date().toLocaleString()} · Primary: UCI Business Administration · Fall 2028</small><h2>Current record</h2><p>Completed UC-transferable units: <b>${completedUnits()}</b> · Enrolled Fall 2026: <b>${enrolledUnits()}</b> · Projected: <b>${projectedUnits()}</b>/60 · Current GPA: <b>${g.gpa?.toFixed(2)||"—"}</b></p><h2>Fall 2026 paid schedule</h2><table><tr><th>Course</th><th>Units</th><th>Meeting</th><th>Instructor</th></tr>${state.courses.filter(c=>c.term==="fall26").map(c=>`<tr><td>${esc(c.code)} — ${esc(c.title)}</td><td>${c.units}</td><td>${esc(c.days)} ${esc(c.time)}</td><td>${esc(c.instructor)}</td></tr>`).join("")}</table><h2>Requirement position</h2><p>UCI prep mapped: <b>${uci.mapped}/${uci.total}</b> · UCLA prep mapped: <b>${ucla.mapped}/${ucla.total}</b> · Cal-GETC areas assigned: <b>${ge.mapped}/11</b>.</p><p>UCI major-prep still to complete: ${esc(openMajor)}.</p><h2>Current route</h2><table><tr><th>Term</th><th>Course</th><th>Units</th><th>Job</th></tr>${TERMS.flatMap(t=>state.courses.filter(c=>c.term===t.id&&ACTIVE(c)).map(c=>`<tr><td>${t.label}</td><td>${esc(c.code)} — ${esc(c.title)}</td><td>${c.units}</td><td>${esc(valueLabel(c))}${c.geTargets.length?" · GE "+esc(c.geTargets.join(", ")):""}</td></tr>`)).join("")}</table><h2>Questions to confirm</h2><ol><li>Does the current prerequisite/placement record require MATH 2 before MATH 7?</li><li>Is MATH 2 expected to be offered in Winter 2027, and is the 6-week format advisable for this student?</li><li>Does the current plan satisfy the exact UCI Business Administration and UCLA Business Economics agreements for the applicable catalog year?</li><li>Is the Cal-GETC Area 4 plan of ECON C2001 + MEDIA 1 valid as two different disciplines?</li><li>Is SMC Scholars/TAP still realistically attainable for the Fall 2028 UCLA application cycle?</li><li>Are all planned Spring 2028 courses expected to be available in the listed terms?</li></ol><h2>Source note</h2><p>Working sources: UCI/UCLA ASSIST 2025–26 major agreements, SMC Cal-GETC 2026–27, SMC UC TCA 2025–26. Verify against the newest agreement before each registration.</p>`;downloadBlob(new Blob([html],{type:"text/html"}),`405-south-counselor-report-${new Date().toISOString().slice(0,10)}.html`);toast("Counselor report exported")
  }
  ...
  function bindStatic(){
    ...
    byId("settingsBtn").onclick=openSettings;byId("searchBtn").onclick=()=>{renderCommand();openOverlay("commandOverlay");setTimeout(()=>byId("commandInput").focus(),40)};byId("commandInput").oninput=e=>renderCommand(e.target.value);
    ...
    document.querySelectorAll("[data-close]").forEach(b=>b.onclick=()=>closeOverlay(b.dataset.close));document.querySelectorAll(".overlay").forEach(o=>o.addEventListener("click",e=>{if(e.target===o)closeOverlay(o.id)}));
  ...
  bindStatic();renderAll();registerSW();
})();
