# 405 South — Transfer Command v4.1
...
- iPhone-first overlay and keyboard fixes: 16px form controls prevent Safari auto-zoom, sheets track the visual viewport, the bottom nav gets out of the keyboard's way, and modal scroll positions restore cleanly.
- Search results now close the search layer before opening a course/screen, exact course-code matches rank first, and the visible close control says **Close** instead of desktop-only **Esc**.
- Playbook accordion titles and descriptions render on separate lines.
- Added persistent **Match iPhone / Light / Dark** appearance controls plus a one-tap header theme button.
- Counselor Report now opens as an in-app preview with **Copy text** and **Print / Save PDF** instead of downloading an HTML file.
- Service-worker cache version bumped so this release replaces stale v4 assets more reliably.
...
This v4.1 build intentionally uses a new storage namespace instead of automatically mixing older v2/v3 planner data into the new verified course model.
